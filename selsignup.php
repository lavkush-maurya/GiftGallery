  ...
  <div class="navd1" style="width:100%;">
        ...
        <ul class="navul2" style="padding-left: 5%;">
          <div style="display: inline-flex;">
            ...
            <li class="navli1">
              <a href="slogin.php" class="b1" style="border:1px solid silver;"> Login</a>
            </li>
          </div>
        </ul>
  </div>
...
<div class="threediv td1">
          ...
          <div class="container a2" style="background: transparent;border: none;">
                <form class="signup" method="POST" enctype="multipart/form-data" style="border: none;">
                  <h1><span style="color: #666;">Seller registration</span></h1><br>
                  <div class="si2">
...
<?php
      if(isset($_POST['create'])){
            ...
            $qr="insert into slogin values('','$fi','$name','$email','$phonenumber','$password','$address','$gender')";
            $run = mysqli_query($conn,$qr);

            if($run)
            { 
              echo "<script> alert('successfully ragistered...');window.location.assign('sdash.php');</script>";
            }
            else{
              echo "<script> alert('failed to register, cheak email, or email already exists');</script>";
            }
